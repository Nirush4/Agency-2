...
export const users = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("users_select_self", {
      ...
      for: "select",
      to: ["authenticated"],
      using: sql`(id = ( SELECT auth.uid() AS uid))`,
    }),
...
export const recipes = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("public_select_published_recipes", {
      as: "permissive",
      for: "select",
      to: ["public"],
      using: sql`(is_published = true)`,
    }),
    pgPolicy("auth_select_own_recipes", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
    pgPolicy("auth_insert_recipes", {
      as: "permissive",
      for: "insert",
      to: ["authenticated"],
    }),
    pgPolicy("auth_update_own_recipes", {
      as: "permissive",
      for: "update",
      to: ["authenticated"],
    }),
    pgPolicy("auth_delete_own_recipes", {
      as: "permissive",
      for: "delete",
      to: ["authenticated"],
    }),
    ...
    pgPolicy("recipes_select_published_or_own", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
...
export const favorites = pgTable(
  ...
  (table) => [
    index("idx_favorites_user").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_favorites_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    pgPolicy("favorites_select_auth_owner", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
      using: sql`(user_id = ( SELECT auth.uid() AS uid))`,
    }),
    pgPolicy("favorites_insert_auth_owner", {
      as: "permissive",
      for: "insert",
      to: ["authenticated"],
    }),
    pgPolicy("favorites_delete_auth_owner", {
      as: "permissive",
      for: "delete",
      to: ["authenticated"],
    }),
